...
#pragma warning disable 0169
    ...
    Quaternion[] initialrotations, aa;
    ...
    void bonechains()
    {
        if (Bones.Length > 0)
        {
            for (int i = 0; i < Bones.Length; i++)
            {
                if (angle < maxAngle && active && aimMode)
                {
                    float percent = (float)(1 + i) / Bones.Length;
                    Quaternion next = Quaternion.Lerp(aa[i], Quaternion.LookRotation(cam.forward, transform.up) * initialrotations[i], percent); //get the -percent of each bones, lower get less rotation final bone (Head get full rotation) 
