package foundation.softwaredesign.orbi.persistence.repo.game.world;
...
public interface ElevationRepository extends EntityRepository<ElevationEntity, Long> {
    ...
    @Query(value = "" +
            "SELECT ST_Value(rast, 1, ST_SetSRID(ST_Point(?2,?1), 4236)) altitude\n" +
            "FROM elevation e\n" +
            "WHERE ST_Intersects(e.rast,ST_SetSRID(ST_Point(?2,?1), 4236))\n" +
